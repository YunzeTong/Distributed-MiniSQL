...
import (
	...
	"Distributed-MiniSQL/minisql/manager/commonutil2"
...
func Main() {
	...
	res := strInterpret("select * from stu2;;")
	fmt.Println("收到的结果: \n" + res)
...
func parseCreateTable(statement string) string {
	...
	ok := false
	if ok, _ = regexp.MatchString("^\\(.*\\)$", statement[startIndex:]); !ok { //check brackets
	...
	for i := 0; i < len(attrsDefine); i++ {                    //for each attribute
		if i == len(attrsDefine)-1 { //last line
			attrParas = strings.Split(strings.Trim(attrsDefine[i], "")[0:len(attrsDefine[i])-1], " ") //remove last ')'
		} else {
